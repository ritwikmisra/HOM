...
+(id)service
{
    static PhoneNumberLoginService *master=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        master=[[PhoneNumberLoginService alloc]initWithService:WEB_SERVICE_PHONE];
    });
    return master;
}
...
-(void)callPhoneNumberLoginServiceWithMobileNumber:(NSString *)strPhoneNumber withCompletionHandler:(WebServiceCompletion)handler
{
    if (appDel.isRechable)
    {
        ...
        [arr addObject:[NSString stringWithFormat:@"phone=%@",strPhoneNumber]];
                ...
                else
                {
                    if (data.length>0)
                    {
                        ...
                        else
                        {
                            ...
                            handler([responseDict objectForKey:@"phoneexist"],NO,nil);
                        }
                    }
                    else
                    {
